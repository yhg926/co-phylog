...
#define HASHSIZE 40000003 //a prime 400000009
#define NLF 0.5 
...
typedef struct objs{unsigned char gcobj[OBJAR]; struct objs *next;} objs_t; //gcobj[i]:genomecode:6bits,obj:2bits
		...
		while(!feof(fp)&&fread(&tuple,8,1,fp)){
            ...
            for(qpi=0;qpi<hcl;qpi++){
                ...
                if(CO[ind].key==0){
                    ...
                    tmp=CO[ind].new->next;
                    CO[ind].new->gcobj[rmod]=(unsigned char)(j<<2)+nobj; // when rd>0,mod==0, mod-1 != (OBJAR+mod-1)%OBJAR,
